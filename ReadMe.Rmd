...
Given that we are looking at long-term trends, it doesn't make much sense to include sites which only have a minimal amount of years were we could extract meaninfulphenological dates. Reasons that sites might not have enough years include: satellites had poor coverage of location (esp true in earlier landsats), too much cloud cover (and thus couldn't fit a nice trend/too few images during growing season), site never "greened-up" threshold during a given year (important esp for Mediterranean sites, and probably other reasons.

Here  is the list of sites which made the cut. Of course, as PIs confirm/add/remove site lat/long, this list will change. 
...
* The goal is to recognize that some growing seasons see bigger temperature and precipitation anomalies, does that influence site phenology/productivity? To look at the temp/precip anomolies in each growing season:
...
This plot simply shows sites which have seen a significant trend in temperatures and precipitaiton over time within a 2 month window preceding the site-specific typical green-up date. 
...
####
CLIMYRS <- tibble(gs = rep(c(1984:2019), nrow(longtermsites)),
       site = rep(longtermsites$site, each = (2019-1983)))
...
- Also interesting to look at yearly deviations in GS length + climate anomoly
```{r climatephenoLOOP}
#all P values
...
ANOM_finalstats <- (dplyr::select(anom_statssite_summary, a, 
                                  sos_sos2motemp_F, gsl_max5motemp_F, maxNDVI_max5motemp_F, gsl_maxNDVI_F,
                                  sos_sos2moprecip_F, gsl_max5moprecip_F, maxNDVI_max5moprecip_F) %>% 
                  gather(Type, Value, -a) %>% mutate(Stat = 'F value')) %>%
  bind_rows((dplyr::select(anom_statssite_summary, a, 
                           sos_sos2motemp_P, gsl_max5motemp_P, maxNDVI_max5motemp_P, gsl_maxNDVI_P,
                           sos_sos2moprecip_P, gsl_max5moprecip_P, maxNDVI_max5moprecip_P) %>% 
               gather(Type, Value, -a) %>% 
               mutate(Stat = 'P value'))) %>%
  bind_rows((dplyr::select(anom_statssite_summary, a, 
                           sos_sos2motemp_SLOPE, gsl_max5motemp_SLOPE, maxNDVI_max5motemp_SLOPE, gsl_maxNDVI_SLOPE,
                           sos_sos2moprecip_SLOPE, gsl_max5moprecip_SLOPE, maxNDVI_max5moprecip_SLOPE) %>% 
               gather(Type, Value, -a) %>% 
               mutate(Stat = 'Slope'))) %>%
  rename(sitenum2 = a) %>%
  mutate(Type = recode(Type, 'sos_sos2motemp_F' = "sosChange_sos2motemp",
                        "sos_sos2motemp_P" = "sosChange_sos2motemp",
                       "sos_sos2motemp_SLOPE" = "sosChange_sos2motemp")) %>%
  mutate(Type = recode(Type, 'gsl_max5motemp_F' = "gslChange_max5motemp",
                        "gsl_max5motemp_P" = "gslChange_max5motemp",
                       "gsl_max5motemp_SLOPE" = "gslChange_max5motemp")) %>%
  mutate(Type = recode(Type, 'maxNDVI_max5motemp_F' = "maxNDVIChange_max5motemp",
                        "maxNDVI_max5motemp_P" = "maxNDVIChange_max5motemp",
                       "maxNDVI_max5motemp_SLOPE" = "maxNDVIChange_max5motemp")) %>%
  mutate(Type = recode(Type, 'gsl_maxNDVI_F' = "gsl_maxNDVI",
                        "gsl_maxNDVI_P" = "gsl_maxNDVI",
                       "gsl_maxNDVI_SLOPE" = "gsl_maxNDVI")) %>%  
 mutate(Type = recode(Type, 'sos_sos2moprecip_F' = "sosChange_sos2moprecip",
                        "sos_sos2moprecip_P" = "sosChange_sos2moprecip",
                       "sos_sos2moprecip_SLOPE" = "sosChange_sos2moprecip")) %>%
  mutate(Type = recode(Type, 'gsl_max5moprecip_F' = "gslChange_max5moprecip",
                        "gsl_max5moprecip_P" = "gslChange_max5moprecip",
                       "gsl_max5moprecip_SLOPE" = "gslChange_max5moprecip")) %>%
  mutate(Type = recode(Type, 'maxNDVI_max5moprecip_F' = "maxNDVIChange_max5moprecip",
                        "maxNDVI_max5moprecip_P" = "maxNDVIChange_max5moprecip",
                       "maxNDVI_max5moprecip_SLOPE" = "maxNDVIChange_max5moprecip")) %>%
...
ANOM_finalstats %>% filter(Type =="gslChange_max5motemp") %>%
  ggplot(aes(x = anom_5moTemp_max, y = ChangeGSL, col = site)) +
  ...
  scale_color_manual(values = rep(c("black"), 99))
ANOM_finalstats %>% filter(Type =="gslChange_max5motemp") %>% 
  group_by(site) %>%
  summarise(gs = min(gs)) %>%
...
(ANOM_finalstats %>% filter(Type =="gslChange_max5moprecip") %>%
  ggplot(aes(x = anom_5moPrecip_max, y = ChangeGSL, col = site, label = site)) +
  geom_point(pch = 21, col = "grey60")+
  guides(col = F)+
  geom_smooth(method = 'lm', data = filter(ANOM_finalstats, Type =="gslChange_max5moprecip",  sig!="o"), fill = NA, lty = 2, lwd = .5) +
  # geom_smooth(method = "lm", data = filter(ANOM_finalstats, sig !="o"), col = "black", fill = NA)+
  theme_cowplot() +
  geom_vline(xintercept = 0, lty = 2, col = "grey60")+
  geom_hline(yintercept = 0, lty = 2, col = "grey60")+
...
ANOM_finalstats %>% filter(Type =="gslChange_max5moprecip") %>% 
  ...
  summarise(gs = min(gs)) %>%
  inner_join(ANOM_finalstats %>%
               filter(Type =="gslChange_max5moprecip")) %>%
  group_by(sig) %>% summarise(gslChange_max5moprecip = n())


ANOM_finalstats %>% filter(Type =="sosChange_sos2motemp") %>%
  ggplot(aes(x = anom_2moTemp_sos, y = ChangeSOS, col = site)) +
  geom_point(pch = 21, col = "grey60")+
  guides(col = F)+
  ...
  geom_vline(xintercept = 0, lty = 2, col = "grey60")+
  geom_hline(yintercept = 0, lty = 2, col = "grey60")+
    scale_color_manual(values = rep(c("black"), 68))+
...
Sites included in this analyses occur across a wide range of locations (elevation, lat/long), soil characteristics (%C, ppmP, %N), climatic variables (MAP, MAT), and plant species richness. 
...
Anova(lmer(ChangeGSL ~ site*gs + (1|gs), data = filter(df_annual, !is.na(ChangeGSL))))
...
There has been a directional, temporal shift towards "greening" and earlier springs since ~1980. 


```{r, echo=F, message=F, warning=F}
fig_ndvi_changes_over_time <- df_merge %>%
  filter(!is.na(Change_ndvi_max)) %>%
  ggplot(aes(y = Change_ndvi_max, x = gs, col = site)) +
  geom_point(pch = 21, col = "grey60")+
  guides(col = F) +
  geom_smooth(method = "lm", fill =NA, lty = 2, lwd = .5, 
              data = filter(df_merge, PhenoTrendXyr_gsl_pvalue <=0.05, !is.na(Change_ndvi_max)),
...
```{r, echo=F, message=F, warning=F}
fig_ndvi_changes_over_time <- df_merge %>%
  filter(!is.na(ChangeGSL)) %>%
  ggplot(aes(y = as.numeric(ChangeGSL), x = TempAnom_5mo_max, col = site)) +
  geom_point(pch = 21, col = "grey60")+
  guides(col = F) +
  geom_smooth(method = "lm", fill =NA, lty = 2, lwd = .5, 
              # data = filter(df_merge, PhenoTrendXyr_gsl_pvalue <=0.05, !is.na(Change_ndvi_max)),
...
Anova(lmer(PhenoTrendXyr_sos_slope ~ #slope of green-up date change thru time
             ...
             (1 | continent),
             data = filter(df_merge, !is.na(PhenoTrendXyr_sos_slope), !is.na(FractionExotic), !is.na(Elevation))))

# #dredge my fishing expedition
# #but it's all unhappy
