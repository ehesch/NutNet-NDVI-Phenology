...
##precip
overall_cc_mod_precip <- ClimChange %>%
  split(.$site) %>%
  map(~lm(`pre_mm/month` ~ plotdate, data = .))

overall_cc_site_precip <- overall_cc_mod_precip %>%
  map(summary)

overall_cc_p_precip <- overall_cc_mod_precip %>%
  map(summary) %>%
  map_dbl(~.$coefficients[2,4]) %>%
  as_tibble() %>%
  rename(OverallPrecip_pvalue = value) %>%
  mutate(site = names(overall_cc_mod_precip))

overall_cc_slope_precip <- overall_cc_mod_precip %>%
  map(summary) %>%
  map_dbl(~.$coefficients[2,1]) %>%
  as_tibble() %>%
  rename(OverallPrecip_slope = value) %>%
  mutate(site = names(overall_cc_mod_precip),
         )

overall_cc <- full_join(overall_cc_p, overall_cc_slope) %>%
  full_join(overall_cc_p_precip) %>%
  full_join(overall_cc_slope_precip)
